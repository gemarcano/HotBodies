...
DoubleTextParserImpl::DoubleTextParserImpl(std::string apFilePath)
:mpFilePath(apFilePath)
{}

DoubleTextParserImpl::~DoubleTextParserImpl()
{}
...
std::unique_ptr<Matrix<int_fast32_t> > DoubleTextParserImpl::parse()
{
	...
	if (!dataRows.empty())
	{
		...
		pImage = std::unique_ptr<Matrix<int_fast32_t> > (new 
Matrix<int_fast32_t> (rows, columns));
